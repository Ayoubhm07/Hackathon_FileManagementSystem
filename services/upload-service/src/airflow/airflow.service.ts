...
export class AirflowService {
  ...
  async triggerPipeline(documentId: string, storagePath: string, mimeType: string, correlationId?: string): Promise<void> {
    ...
    await axios.post(
      `${airflowUrl}/api/v1/dags/docflow_pipeline/dagRuns`,
      {
        ...
        conf: {
          ...
          mime_type: mimeType,
          correlation_id: correlationId ?? documentId,
        },
      },
      { auth: { username, password }, timeout: 10000 },
    );
    this.logger.log(`Airflow DAG triggered: ${dagRunId} mime_type=${mimeType}`);
  }
}
